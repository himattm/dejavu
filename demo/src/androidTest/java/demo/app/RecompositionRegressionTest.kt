...
class RecompositionRegressionTest {
    ...
    fun cartBanner_recomposesOnUnrelatedRefresh() {
        ...
        // is created. Because CartSummary is now a data class, the new
        // instance == the old instance (structural equality), so
        // CartBanner stays stable even though the parent recomposes.
        composeTestRule.onNodeWithTag("refresh_button").performClick()

        // FIXED: CartBanner stays stable as there is no logical change to the cart
    ...
    fun performanceBudget_cartBannerOnMixedInteractions() {
        ...
        // FIXED BEHAVIOR: CartBanner no longer recomposes on EVERY parent
        // recomposition because CartSummary is stable.
        // Only the 2 select clicks cause recomposition (the content actually changes).
